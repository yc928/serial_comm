...
class TKUPacket():
    ...
    def update_walk_packet(self, walk_info):
        def get_hi_lo_byte(value):
            if value < 0:
                hi_byte = (-value >> 8) & 0xFF | 0x80
                lo_byte = (-value) & 0xFF
        ...
        self.walk_packet[14] = walk_info.walking_cmd
    ...
    def update_params_packet(self, walking_mode):
        ...
        if walking_mode == 1: #0x00: single, 0x01:continue, 0x02:LC
            ...
            _, self.lipm_packet[17] = get_multiple_hi_lo_byte(self.walk_params['continuous']['T_DSP'])        
            ...
            self.lc_packet[6], self.lc_packet[7] = get_multiple_hi_lo_byte(self.walk_params[walk_mode]['x_swing'])
            self.lc_packet[8], self.lc_packet[9] = get_multiple_hi_lo_byte(self.walk_params[walk_mode]['y_swing'])
            self.lc_packet[10], self.lc_packet[11] = get_multiple_hi_lo_byte(self.walk_params[walk_mode]['z_swing'])
            ...
            _, self.lc_packet[17] = get_multiple_hi_lo_byte(self.walk_params[walk_mode]['lock_range'])
    ...
    def save_params(self, params_info):
        if params_info.mode == 1:
            continuous_params = {
                        ...
                        'T_DSP': params_info.osc_lockrange,
                        'base_default_Z': params_info.base_default_z,
                        'walking_mode': params_info.mode
                    }
            self.walk_params['continuous_params'] = continuous_params
            with open('./src/serial_sever/continuous_params.json', 'w') as f:
                json.dump(continuous_params, f)
        elif params_info.mode == 2:
            lc_up_params = {
                        'x_swing': params_info.x_swing_range,
                        'y_swing': params_info.y_swing_range,
                        'z_swing': params_info.z_swing_range,
                        'period_T': params_info.period_t,
                        'period_T2': params_info.period_t2,
                        'sample_time': params_info.sample_time,
                        'lock_range': params_info.osc_lockrange,
                        'base_default_Z': params_info.base_default_z,
            ...
            self.walk_params['lc_up_params'] = lc_up_params
            with open('./src/serial_sever/lc_up_params.json', 'w') as f:
            ...
            self.walk_params['lc_down_params'] = lc_down_params
            with open('./src/serial_sever/lc_down_params.json', 'w') as f:
                json.dump(lc_down_params, f)

    def load_params(self, params_name):
            with open('./src/serial_sever/' + params_name + '.json', 'r') as f:
                params = json.load(f)
